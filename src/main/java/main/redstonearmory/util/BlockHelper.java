...
public final class BlockHelper {
    ...
    public static MovingObjectPosition getCurrentMovingObjectPosition(EntityPlayer player) {
        ...
        return player.worldObj.rayTraceBlocks_do_do(posVec, lookVec, true, true);
    }
    ...
    public static boolean isEqual(Block blockA, Block blockB) {
        ...
        if (blockA.blockID == blockB.blockID) {
            return true;
        }
        if (blockA == null | blockB == null) {
            return false;
        }
        return blockA.equals(blockB) || blockA.isAssociatedBlockID(blockB.blockID);
    }
    ...
    public static boolean canRotate(Block block) {
        ...
        return rotateType[block.blockID] != 0;
    }

    public static int rotateVanillaBlock(World world, Block block, int x, int y, int z) {

        int bId = block.blockID, bMeta = world.getBlockMetadata(x, y, z);
        switch (rotateType[bId]) {
            ...
            case RotationType.CHEST:
                ...
                for (int i = 2; i < 6; i++) {
                    ...
                    if (isEqual(Block.blocksList[world.getBlockId(coords[0], coords[1], coords[2])], block)) {
    ...
    public static List<ItemStack> breakBlock(World worldObj, int x, int y, int z, Block block, int fortune, boolean doBreak, boolean silkTouch) {
        ...
        if (silkTouch && block.canSilkHarvest(worldObj, null, x, y, z, meta)) {
            ...
            stacks = block.getBlockDropped(worldObj, x, y, z, meta, fortune);
        }
        if (!doBreak) {
            return stacks;
        }
        worldObj.playAuxSFXAtEntity(null, 2001, x, y, z, block.blockID + (meta << 12));
