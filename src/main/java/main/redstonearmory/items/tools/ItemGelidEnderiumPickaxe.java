...
public class ItemGelidEnderiumPickaxe extends ItemToolRF {
	...
	public ItemGelidEnderiumPickaxe(EnumToolMaterial toolMaterial) {
	...
	public ItemGelidEnderiumPickaxe(EnumToolMaterial toolMaterial, int harvestLevel) {
			...
			for (int i = x - 1; i <= x + 1; i++) {
				for (int k = z - 1; k <= z + 1; k++) {
					for (int j = y - 1; j <= y + 1; j++) {
						if (world.getBlock(i, j, k) == Block.cobblestone || world.getBlock(i, j, k) == Block.stone || world.getBlock(i, j, k) == Block.sandStone || world.getBlock(i, j, k) == Block.netherrack) {
