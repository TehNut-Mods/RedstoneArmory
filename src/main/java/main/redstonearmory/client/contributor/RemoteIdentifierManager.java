...
public class RemoteIdentifierManager {
	...
	protected void retrieve() {
		...
		try {
			...
			RedstoneArmory.logger.log(Level.INFO, "Found " + getFileName() + ".txt. Loading.");
			...
			while ((line = bufferedReader.readLine()) != null) {
				if (!line.startsWith("//") && !line.isEmpty()) {
					if (line.contains(":")) {
						...
						RedstoneArmory.logger.log(Level.WARN, "[" + getFileName() + ".txt] Syntax error on line " + linetracker + ": " + line);
					}
				}
				linetracker++;
			}
